...
export const dashboardApi = baseApi.injectEndpoints({
  endpoints: (builder) => ({
      ...
      queryFn: async () => {
        try {
          ...
          const creditTypes = new Set(
            txTypes.filter((t) => t.direction === 'credit').map((t) => t.id!),
          );
          ...
          const monthlyIncome = monthlyTxs
            .filter((t) => creditTypes.has(t.transactionTypeId))
            .reduce((s, t) => s + t.amount, 0);
          const monthlyExpense = monthlyTxs
            .filter((t) => debitTypes.has(t.transactionTypeId))
            .reduce((s, t) => s + t.amount, 0);
    ...
    getDashboardWidgets: builder.query<DashboardWidget[], void>({
      queryFn: async () => {
        try {
          return { data: await db.dashboardWidgets.filter((w) => w.isActive !== false).toArray() };
        } catch (e) {
          return { error: { status: 'CUSTOM_ERROR', error: String(e) } };
        }
      },
    ...
    getAllDashboardWidgets: builder.query<DashboardWidget[], void>({
      queryFn: async () => {
        try {
          return { data: await db.dashboardWidgets.toArray() };
        } catch (e) {
          return { error: { status: 'CUSTOM_ERROR', error: String(e) } };
        }
      },
    ...
    getUserDashboardConfig: builder.query<
      (UserDashboardConfig & { widget?: DashboardWidget })[],
      void
    >({
      queryFn: async () => {
        try {
    ...
    >({
      queryFn: async (layout) => {
        try {
          ...
          await Promise.all(
            layout.map((item) =>
              db.userDashboardConfig.update(item.id, { ...item, updatedAt: ts }),
            ),
          );
          return { data: undefined };
        } catch (e) {
    ...
    createDashboardWidget: builder.mutation<
      number,
