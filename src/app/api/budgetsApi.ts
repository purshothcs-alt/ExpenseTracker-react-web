...
export const budgetsApi = baseApi.injectEndpoints({
  endpoints: builder => ({
      ...
      queryFn: async () => {
        try {
          ...
          const [categories, accounts, projects, budgetTypes] = await Promise.all([
            db.categories.toArray(), db.accounts.toArray(),
            db.projects.toArray(), db.budgetTypes.toArray(),
          ]);
            ...
            budgets.map(async b => {
              const txs = await db.transactions
                .filter(t =>
                  ...
                  (b.categoryId ? (t.categoryId === b.categoryId || t.subCategoryId === b.categoryId) : true) &&
              ...
              return {
                ...
                spent, remaining, utilizationPct,
              };
            }),
          );
          return { data };
    ...
    getAllBudgets: builder.query<Budget[], void>({
      queryFn: async () => {
        try { return { data: await db.budgets.toArray() }; }
        catch (e) { return { error: { status: 'CUSTOM_ERROR', error: String(e) } }; }
      },
      providesTags: ['Budget'],
    }),
    ...
    getBudgetTypes: builder.query<BudgetType[], void>({
      queryFn: async () => {
        try { return { data: await db.budgetTypes.toArray() }; }
        catch (e) { return { error: { status: 'CUSTOM_ERROR', error: String(e) } }; }
      },
...
export const {
  useGetBudgetsQuery, useGetAllBudgetsQuery,
  useCreateBudgetMutation, useUpdateBudgetMutation, useDeleteBudgetMutation,
  useGetBudgetTypesQuery, useCreateBudgetTypeMutation,
} = budgetsApi;
