...
export const assetsApi = baseApi.injectEndpoints({
  endpoints: builder => ({
    getAssets: builder.query<AssetWithType[], void>({
      queryFn: async () => {
        try {
          ...
          const data: AssetWithType[] = assets.map(a => ({ ...a, assetType: typeMap.get(a.assetTypeId) }));
          return { data };
    ...
    getAllAssets: builder.query<Asset[], void>({
      queryFn: async () => {
        try { return { data: await db.assets.toArray() }; }
        catch (e) { return { error: { status: 'CUSTOM_ERROR', error: String(e) } }; }
      },
      providesTags: ['Asset'],
    }),
    ...
    getAssetValuations: builder.query<AssetValuation[], number>({
      queryFn: async (assetId) => {
        try {
          const data = await db.assetValuations.where('assetId').equals(assetId).sortBy('valuationDate');
          return { data };
        } catch (e) { return { error: { status: 'CUSTOM_ERROR', error: String(e) } }; }
      },
      ...
      queryFn: async (data) => {
        try {
          ...
          if (data.purchaseDate) {
            await db.assetValuations.add({ assetId: id as number, valuationDate: data.purchaseDate, value: data.purchaseValue, notes: 'Purchase value', createdAt: ts, updatedAt: ts });
          }
    ...
    addAssetValuation: builder.mutation<number, Omit<AssetValuation, 'id' | 'createdAt' | 'updatedAt'>>({
      queryFn: async (data) => {
        try {
          const ts = new Date().toISOString();
    ...
    getAssetTypes: builder.query<AssetType[], void>({
      queryFn: async () => {
        try { return { data: await db.assetTypes.filter(t => t.isActive !== false).toArray() }; }
        catch (e) { return { error: { status: 'CUSTOM_ERROR', error: String(e) } }; }
      },
    ...
    getAllAssetTypes: builder.query<AssetType[], void>({
      queryFn: async () => {
        try { return { data: await db.assetTypes.toArray() }; }
        catch (e) { return { error: { status: 'CUSTOM_ERROR', error: String(e) } }; }
      },
      providesTags: ['AssetType'],
    }),
...
export const {
  useGetAssetsQuery, useGetAllAssetsQuery, useGetAssetValuationsQuery,
  useCreateAssetMutation, useUpdateAssetMutation, useDeleteAssetMutation,
  useAddAssetValuationMutation,
  useGetAssetTypesQuery, useGetAllAssetTypesQuery,
  useCreateAssetTypeMutation, useUpdateAssetTypeMutation, useDeleteAssetTypeMutation,
} = assetsApi;
