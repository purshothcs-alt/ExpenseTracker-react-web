...
import type {
  Transaction, TransactionFilter, PaginatedResult, TransactionWithDetails,
  TransactionType,
...
export const transactionsApi = baseApi.injectEndpoints({
  endpoints: builder => ({
      ...
      queryFn: async (limit = 10) => {
        try {
          const txs = await db.transactions
            ...
            .limit(limit)
            .toArray();
          const enriched = await Promise.all(txs.map(t => transactionRepository.enrichTransaction(t)));
    ...
    getMonthlySummary: builder.query<Array<{ month: string; income: number; expense: number }>, number>({
    ...
    getCategoryTotals: builder.query<Array<{ categoryId: number; total: number }>, { startDate: string; endDate: string }>({
    ...
    createTransaction: builder.mutation<number, { data: Omit<Transaction, 'id' | 'createdAt' | 'updatedAt'>; tagIds?: number[] }>({
    ...
    updateTransaction: builder.mutation<void, { id: number; data: Partial<Transaction>; tagIds?: number[] }>({
    ...
    duplicateTransaction: builder.mutation<number, number>({
      queryFn: async (id) => {
        try {
          ...
          const newId = await transactionRepository.create({ ...rest, transactionDate: new Date().toISOString().split('T')[0] });
          return { data: newId };
        } catch (e) { return { error: { status: 'CUSTOM_ERROR', error: String(e) } }; }
      },
      invalidatesTags: ['Transaction', 'Account'],
    ...
    getTransactionTypes: builder.query<TransactionType[], void>({
      queryFn: async () => {
        try { return { data: await db.transactionTypes.filter(t => t.isActive !== false).toArray() }; }
        catch (e) { return { error: { status: 'CUSTOM_ERROR', error: String(e) } }; }
      },
    ...
    getAllTransactionTypes: builder.query<TransactionType[], void>({
      queryFn: async () => {
        try { return { data: await db.transactionTypes.toArray() }; }
        catch (e) { return { error: { status: 'CUSTOM_ERROR', error: String(e) } }; }
      },
      providesTags: ['TransactionType'],
    }),

    createTransactionType: builder.mutation<number, Omit<TransactionType, 'id' | 'createdAt' | 'updatedAt'>>({
      queryFn: async (data) => {
        try {
...
export const {
  useGetTransactionsQuery, useGetTransactionByIdQuery, useGetRecentTransactionsQuery,
  useGetMonthlySummaryQuery, useGetCategoryTotalsQuery,
  ...
  useGetTransactionTypesQuery, useGetAllTransactionTypesQuery,
