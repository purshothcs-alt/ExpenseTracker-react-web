...
export async function seedDatabase(): Promise<void> {
  ...
  await db.transaction(
    ...
    async () => {
      ...
      const accountTypesWithIds = seedAccountTypes.map((t, i) => ({
        ...t,
        id: accountTypeIds[i] as number,
      }));

      const txTypeIds = await db.transactionTypes.bulkAdd(seedTransactionTypes, { allKeys: true });
      const txTypesWithIds = seedTransactionTypes.map((t, i) => ({
        ...t,
        id: txTypeIds[i] as number,
      }));
      ...
      const catsWithIds: Category[] = seedCategories.map((c, i) => ({
        ...c,
        id: catIds[i] as number,
      }));
      const catMap: Record<string, number> = {};
      catsWithIds.forEach((c) => {
        catMap[c.name] = c.id!;
      });
      ...
      const dashboardConfigs = defaultWidgets.map((w, i) => ({
        ...
        width:
          w.componentKey === 'income-expense-chart'
            ? 3
            : ['recent-transactions'].includes(w.componentKey)
              ? 2
              : 1,
        height: [
          'recent-transactions',
