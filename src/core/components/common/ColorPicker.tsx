...
export function ColorPicker({ value, onChange, label }: Props) {
  ...
  return (
    <Box>
      {label && <Box component="label" sx={{ fontSize: '0.75rem', color: 'text.secondary', display: 'block', mb: 0.5 }}>{label}</Box>}
      <Box
        sx={{
          width: 36, height: 36, borderRadius: 1.5, cursor: 'pointer',
          bgcolor: value, border: '2px solid', borderColor: 'divider',
          ...
          <Box display="flex" flexWrap="wrap" gap={0.5} maxWidth={180} mb={1.5}>
            {PRESET_COLORS.map(c => (
              <Box
                key={c}
                sx={{
                  width: 24, height: 24, borderRadius: 0.75, cursor: 'pointer',
                  bgcolor: c, border: value === c ? '2px solid #000' : '1px solid transparent',
                  '&:hover': { opacity: 0.8 },
                }}
