...
export function AssetsPage() {
  ...
  const onSubmit = async (data: FormData) => {
    try {
      if (editAsset?.id) {
        await update({ id: editAsset.id, data }).unwrap();
        enqueueSnackbar('Asset updated', { variant: 'success' });
      } else {
        await create(data as Omit<Asset, 'id' | 'createdAt' | 'updatedAt'>).unwrap();
  ...
  return (
    <Box>
      ...
      {!isLoading && assets.length === 0 ? (
        <EmptyState
          title="No assets yet"
          description="Track your assets and their valuations."
          action={{ label: 'Add Asset', onClick: () => openForm() }}
        />
      ) : (
                  ...
                  <CardContent>
                    <Box display="flex" justifyContent="space-between" alignItems="flex-start">
                      <Box display="flex" alignItems="center" gap={1.5}>
                        <Avatar
                          sx={{
                            bgcolor: `${assetType?.color || '#9E9E9E'}20`,
                            color: assetType?.color || '#9E9E9E',
                            width: 40,
                            height: 40,
                          }}
                        >
                      ...
                      <Box>
                        ...
                        <IconButton size="small" onClick={() => openForm(asset)}>
                    ...
                    <Box mt={2}>
                      ...
                      <Typography variant="caption" color="text.secondary">
                        Cost: {formatCurrency(asset.purchaseValue)}
                      </Typography>
                    </Box>
        ...
        <form onSubmit={handleSubmit(onSubmit)}>
            ...
            <Grid container spacing={2}>
              ...
              <Grid size={3}>
                <Controller
                  ...
                  control={control}
                  render={({ field }) => (
                    <TextField {...field} fullWidth label="Unit" value={field.value || ''} />
                  )}
                />
              </Grid>
              <Grid size={12}>
          ...
          <DialogActions sx={{ px: 3, pb: 2 }}>
            ...
            <Button type="submit" variant="contained" disabled={creating || updating}>
              {creating || updating ? 'Saving...' : editAsset ? 'Update' : 'Create'}
            </Button>
          </DialogActions>
        </form>
      </Dialog>
      ...
      <Dialog
        open={!!valuationAsset}
        onClose={() => setValuationAsset(undefined)}
        maxWidth="xs"
        fullWidth
      >
        ...
        <DialogActions sx={{ px: 3, pb: 2 }}>
          ...
          <Button variant="contained" onClick={handleValuation}>
            Update
          </Button>
        </DialogActions>
      </Dialog>
      ...
      <ConfirmDialog
        open={deleteId !== null}
        title="Delete Asset"
        message="Delete this asset and all valuation history?"
        onConfirm={async () => {
