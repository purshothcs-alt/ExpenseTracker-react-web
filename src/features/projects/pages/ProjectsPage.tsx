...
function ProjectExpensesPanel({ projectId }: { projectId: number }) {
  ...
  const onSubmit = async (data: ExpenseForm) => {
    ...
    await addExpense({ projectId, ...rest, categoryId: categoryId ?? undefined } as Omit<
      ProjectExpense,
            ...
            <Grid container spacing={2}>
              ...
              <Grid size={6}>
                <Controller
                  ...
                  control={control}
                  render={({ field }) => (
                    <TextField {...field} fullWidth label="Vendor" value={field.value || ''} />
                  )}
                />
              </Grid>
            </Grid>
...
export function ProjectsPage() {
  ...
  const onSubmit = async (data: ProjectForm) => {
    const { categoryId, ...rest } = data;
    const projectData = { ...rest, categoryId: categoryId ?? undefined } as Omit<
      Project,
      'id' | 'createdAt' | 'updatedAt'
    >;
    try {
      if (editProject?.id) {
        await update({ id: editProject.id, data: projectData }).unwrap();
        enqueueSnackbar('Project updated', { variant: 'success' });
      } else {
        await create(projectData).unwrap();
    ...
    <Box>
      ...
      {!isLoading && projects.length === 0 ? (
        <EmptyState
          ...
          description="Track construction, renovation, and other project expenses."
                  ...
                  <CardContent>
                    <Box display="flex" justifyContent="space-between" alignItems="flex-start">
                      ...
                      <Box>
                        ...
                        <IconButton size="small" onClick={() => openForm(project)}>
                    ...
                    <Box mt={2}>
                      ...
                      <LinearProgress
                        ...
                        sx={{
                          ...
                          '& .MuiLinearProgress-bar': {
                            bgcolor: pct > 100 ? '#DC2626' : statusColor,
                          },
                        }}
                      />
                      <Box display="flex" justifyContent="space-between" mt={0.25}>
                        <Typography
                          variant="caption"
                          color={project.budgetVariance! < 0 ? 'error.main' : 'success.main'}
                          fontWeight={600}
                        >
                          {project.budgetVariance! >= 0 ? 'Under by: ' : 'Over by: '}
                          {formatCurrency(Math.abs(project.budgetVariance || 0))}
                        </Typography>
                    ...
                    {isExpanded && (
                      <Box mt={2} pt={2} borderTop="1px solid" borderColor="divider">
                        <Tabs
                          value={tab[project.id!] || 0}
                          onChange={(_, v) => setTab((prev) => ({ ...prev, [project.id!]: v }))}
                          variant="fullWidth"
                        >
                          <Tab label="Expenses" />
                        </Tabs>
        ...
        <form onSubmit={handleSubmit(onSubmit)}>
          ...
          <DialogActions sx={{ px: 3, pb: 2 }}>
            ...
            <Button type="submit" variant="contained" disabled={creating || updating}>
              {creating || updating ? 'Saving...' : editProject ? 'Update' : 'Create'}
            </Button>
          </DialogActions>
        </form>
      </Dialog>
      ...
      <ConfirmDialog
        ...
        message="This will also delete all project expenses. Proceed?"
        onConfirm={async () => {
