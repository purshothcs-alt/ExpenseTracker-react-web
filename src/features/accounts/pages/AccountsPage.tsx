import {
  ...
  TextField, MenuItem, IconButton, Tooltip, Chip,
...
import {
  useGetAccountsQuery, useGetAccountTypesQuery,
...
export function AccountsPage() {
  ...
  const openForm = (account?: Account) => {
    ...
    reset(account ? { ...account, openingBalance: account.openingBalance ?? 0 } : { openingBalance: 0, currency: 'INR', isActive: true });
    setFormOpen(true);
  };
  ...
  return (
    <Box>
                  ...
                  <CardContent sx={{ pt: 2 }}>
                    <Box display="flex" justifyContent="space-between" alignItems="flex-start">
                      <Box display="flex" alignItems="center" gap={1.5}>
                        <Avatar sx={{ bgcolor: `${accountType?.color || '#9E9E9E'}20`, color: accountType?.color || '#9E9E9E', width: 40, height: 40 }}>
                          {account.name[0]}
                        </Avatar>
                        <Box>
                          <Typography variant="subtitle2" fontWeight={700}>{account.name}</Typography>
                          <Chip label={accountType?.name || 'Unknown'} size="small" sx={{ height: 18, fontSize: '0.65rem' }} />
                        </Box>
                      </Box>
                      <Box>
                        <Tooltip title="Edit">
                          <IconButton size="small" onClick={() => openForm(account)}><EditIcon fontSize="small" /></IconButton>
                        </Tooltip>
                        <Tooltip title="Delete">
                          <IconButton size="small" color="error" onClick={() => setDeleteId(account.id!)}><DeleteIcon fontSize="small" /></IconButton>
                        </Tooltip>
                      </Box>
                    </Box>
                    ...
                    <Box mt={2}>
                      ...
                      <Typography variant="h5" fontWeight={800} color={account.currentBalance < 0 ? 'error.main' : 'text.primary'}>
                        {formatCurrency(account.currentBalance)}
                      </Typography>
                      <Box display="flex" alignItems="center" gap={0.5} mt={0.5}>
                        <Typography variant="caption" color="text.secondary">Opening: {formatCurrency(account.openingBalance)}</Typography>
                        <Typography variant="caption" color={gain >= 0 ? 'success.main' : 'error.main'} fontWeight={600}>
            ...
            <Grid container spacing={2}>
              <Grid size={12}>
                <Controller name="name" control={control} render={({ field }) => (
                  <TextField {...field} fullWidth label="Account Name *" error={!!errors.name} helperText={errors.name?.message} />
                )} />
              </Grid>
              <Grid size={12}>
                <Controller name="accountTypeId" control={control} render={({ field }) => (
                  <TextField {...field} select fullWidth label="Account Type *" error={!!errors.accountTypeId} helperText={errors.accountTypeId?.message} value={field.value || ''} onChange={e => field.onChange(Number(e.target.value))}>
              ...
              <Grid size={6}>
                <Controller name="openingBalance" control={control} render={({ field }) => (
                  <TextField {...field} type="number" fullWidth label="Opening Balance" value={field.value ?? 0} onChange={e => field.onChange(parseFloat(e.target.value) || 0)} />
                )} />
              </Grid>
              <Grid size={6}>
                <Controller name="currency" control={control} render={({ field }) => (
                  <TextField {...field} fullWidth label="Currency" />
                )} />
              </Grid>
              <Grid size={12}>
                <Controller name="description" control={control} render={({ field }) => (
                  <TextField {...field} fullWidth label="Description (optional)" multiline rows={2} value={field.value || ''} />
                )} />
              </Grid>
            </Grid>
          </DialogContent>
      ...
      <ConfirmDialog
        ...
        onConfirm={async () => { await deleteAccount(deleteId!).unwrap(); enqueueSnackbar('Account deleted', { variant: 'success' }); setDeleteId(null); }}
