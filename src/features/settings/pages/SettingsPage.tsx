    ...
    <Box>
      ...
      <Grid container spacing={2}>
        ...
        <Grid size={{ xs: 12, md: 6 }}>
          <Card>
            <CardContent>
              <Typography variant="subtitle1" fontWeight={700} mb={2}>Currency</Typography>
              <Box display="flex" flexDirection="column" gap={2}>
                <TextField
                  select
                  label="Currency"
                  ...
                  {CURRENCIES.map(c => (
                    <MenuItem key={c.code} value={c.code}>
                      <Box display="flex" alignItems="center" gap={1}>
                        <Chip label={c.symbol} size="small" sx={{ fontWeight: 700, minWidth: 36 }} />
                        {c.name} ({c.code})
                      </Box>
                    </MenuItem>
                  ))}
                </TextField>
                <Box display="flex" alignItems="center" gap={1.5} p={1.5} bgcolor="action.hover" borderRadius={1}>
                  <Avatar sx={{ width: 36, height: 36, bgcolor: 'primary.main', fontSize: 18 }}>
                    {settings.currencySymbol || '₹'}
                  </Avatar>
                  <Box>
                    <Typography variant="body2" fontWeight={600}>{settings.currency || 'INR'}</Typography>
          ...
          <Card>
            <CardContent>
              ...
              <Box mt={2} display="flex" gap={1} flexWrap="wrap">
                {['React 19', 'TypeScript', 'MUI v7', 'Redux Toolkit', 'Dexie.js', 'PWA'].map(t => (
