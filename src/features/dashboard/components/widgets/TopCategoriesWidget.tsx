import { Card, CardContent, Typography, Box, LinearProgress, Avatar, Skeleton } from '@mui/material';
...
export function TopCategoriesWidget({ limit = 6 }: { limit?: number }) {
  const { data: totals = [], isLoading: loadingTotals } = useGetCategoryTotalsQuery({ startDate, endDate });
  ...
  return (
    <Card sx={{ height: '100%' }}>
      <CardContent>
        <Typography variant="subtitle1" fontWeight={600} mb={2}>Top Spending Categories</Typography>
        {loadingTotals ? (
          Array.from({ length: 5 }).map((_, i) => (
            <Box key={i} mb={1.5}><Skeleton variant="rounded" height={40} /></Box>
          ))
        ) : topItems.length === 0 ? (
          <Typography color="text.secondary" variant="body2">No expense data this month</Typography>
        ) : (
          topItems.map(item => {
